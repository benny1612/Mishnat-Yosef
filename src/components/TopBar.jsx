...
export default function TopBar({ orderMeta, allPhysicalItems, statusMap, viewMode, onToggleView, onLogout, userName }) {
  ...
  return (
    <div className="sticky top-0 z-50 bg-white shadow-md border-b border-gray-100">
      {/* Main header row */}
      <div className="bg-gradient-to-l from-green-800 to-emerald-700 text-white px-4 py-3">
        <div className="flex items-center justify-between gap-3 max-w-2xl mx-auto">
          {/* Title & address */}
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xl">🛒</span>
              <h1 className="font-bold text-base leading-tight truncate">{orderMeta?.name || 'טוען...'}</h1>
            </div>
            {orderMeta?.street && (
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              id="logout-btn"
              onClick={onLogout}
              className="bg-white/15 hover:bg-white/25 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-all duration-200 border border-white/20 flex items-center gap-1"
              title="התנתקות"
            >
              <span>יציאה</span>
              <span>👋</span>
        ...
        {(orderMeta?.times || collectionDate) && (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1.5 px-0 max-w-2xl mx-auto">
            {collectionDate && (
      ...
      <div className="bg-white px-4 py-2.5 max-w-2xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500">נאספו</span>
                {stats.missing > 0 && (
              ...
              <span className="text-xs font-bold text-green-700">{stats.percent}%</span>
            </div>
            {/* Progress bar */}
          ...
          <button
            ...
            className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl font-bold text-xs transition-all duration-300 shadow-sm border ${
              viewMode === 'swipe'
                ? 'bg-purple-600 text-white border-purple-500 shadow-purple-200'
                : 'bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200'
            }`}
          >
            <span className="text-base">{viewMode === 'swipe' ? '📋' : '🃏'}</span>
            <span className="hidden xs:inline">{viewMode === 'swipe' ? 'רשימה' : 'סוויפ'}</span>
          </button>
        </div>
      </div>
    </div>
