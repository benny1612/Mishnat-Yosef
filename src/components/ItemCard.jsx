...
function ProductImage({ item, size = 'md', onClick }) {
  ...
  const sizeClass   = size === 'sm' ? 'w-12 h-12' : size === 'lg' ? 'w-48 h-48' : 'w-16 h-16';
  ...
  if (!currentSrc || phase === 99) {
    return (
      <div
        ...
        style={{ minWidth: size === 'lg' ? 192 : size === 'sm' ? 48 : 64 }}
      >
        <span className={size === 'lg' ? 'text-6xl' : 'text-2xl'}>🛍️</span>
      </div>
    );
  }
  ...
  return (
    <>
      <motion.div
        ...
        className={`
          relative flex items-center gap-3 p-3 rounded-2xl border-2 transition-all duration-300
          ${cfg.bg} ${cfg.border}
      ...
      >
        ...
        <div className={`absolute right-0 top-3 bottom-3 w-1 rounded-full ${cfg.strip}`} />
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.unitsDisplay && (
            <p className="text-xs text-blue-600 font-medium mt-0.5">{item.unitsDisplay}</p>
          )}
          ...
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            ...
            {item.madeInIsrael && (
              <span className="text-xs bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded-full border border-blue-200">
                🇮🇱
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          ...
          <motion.button
            ...
            title="סמן כנאסף"
            className={`w-10 h-10 rounded-xl font-bold text-lg flex items-center justify-center transition-all duration-200 shadow-sm ${
              status === 'collected'
